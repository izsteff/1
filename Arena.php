...
class Arena implements Listener{
    ...
    public function tapJoinSign(PlayerInteractEvent $e){
        $b = $e->getBlock();
        $p = $e->getPlayer();
        if($p->hasPermission("bg.sign") || $p->isOp()){
    ...
    public function joinToArena(Player $p){
        if($p->hasPermission("bg.acces") || $p->isOp()){
            if($this->setup === true){
            ...
            $this->saveInv($p);
            $p->teleport(new Position($this->data['arena']['lobby_position_x'], $this->data['arena']['lobby_position_y'], $this->data['arena']['lobby_position_z'], $this->plugin->getServer()->getLevelByName($this->data['arena']['lobby_position_world'])));
            $p->sendMessage($this->plugin->getPrefix().$this->plugin->getMsg('join'));
			$this->lobbyp[strtolower($p->getName())] = $p;
			$p->addTitle("Battle Ground", "", 20, 40, 20);
			$p->removeAllEffects();
            $vars = ['%1'];
            $replace = [$p->getName()];
	...
	public function openIngame($p){
		if($p instanceof Player){
			$api = $this->plugin->getServer()->getPluginManager()->getPlugin("FormAPI");
			$form = $api->createSimpleForm(function (Player $p, array $data){
			$result = $data[0];
			...
			if($result === 0){
				...
				$compassName = $compass->setCustomName(TextFormat::AQUA . "Leave Game");
			...
			$form->setTitle(TextFormat::BOLD . "BATTLEGROUND");
			$form->setContent("The game is currently running. Do you want to spectate?\n\n\n\n\n\n\n\n");
			$form->addButton("YES");
			$form->addButton("NO");
			$form->sendToPlayer($p);
		}
	...
	public function setIngame(){
		foreach($this->lobbyp as $p){
			...
			// TODO - SELL CUSTOM AXE DESIGN
			// SET CUSTOM AXE FOR VIP PLAYERS
		}
	}
        ...
        if(count($this->ingamep) <= 1){
            if(count($this->ingamep) === 1){
                foreach($this->ingamep as $p){
					$p->addTitle("YOU WON", "", 20, 40, 20);
					foreach($this->plugin->getServer()->getOnlinePlayers() as $player) {
						$winnerName = $p->getName();
						$player->sendMessage(TextFormat::BOLD . TextFormat::AQUA . $winnerName . TextFormat::GREEN . " won in " . TextFormat::YELLOW . "BATTLE GROUND");
					}
					
					// TODO - GIVE REWARD EVERY EVENT
					// EVENT SCHEDULE - 9am, 12pm, 3pm, 6pm, 9pm,
	...
	// OR CHANGE INTO SNEAK - ADD MORE CROSSER - ZOOM
	public function onHand(PlayerItemHeldEvent $e) {
		...
		if($p instanceof Player){
			...
			if($this->getPlayerMode($p) === 2){
				if($item->getId() == 355 && $item->getCustomName() == TextFormat:: AQUA . "Leave Game"){
					$this->leaveArena($p);
				}
	...
	public function onUsePotionKits(PlayerInteractEvent $e){
		...
		if($p instanceof Player){
			if($this->getPlayerMode($p) === 1){
				...
				// MEDKIT - Gold Apple
				if($item->getId() == 322 && $item->getCustomName() == TextFormat:: AQUA . "MedKit"){
					$e->setCancelled();
					$item = Item::get(322, 0, 1);
					$itemName = $item->setCustomName(TextFormat::AQUA . "MedKit");
					$p->getInventory()->removeItem($itemName);
					$p->setHealth(20);
				}
				
				// BUSH - Dead Bush
				if($item->getId() == 32 && $item->getCustomName() == TextFormat:: AQUA . "Bush"){
					$e->setCancelled();
					$item = Item::get(32, 0, 1);
					$itemName = $item->setCustomName(TextFormat::AQUA . "Bush");
					$p->getInventory()->removeItem($itemName);
	...
	public function onGrenade(ProjectileHitEvent $e) {
		$projectile = $e->getEntity();
		if($projectile instanceof Egg) {
			$explosion = new Explosion($projectile->getPosition(), 5);
			$explosion->explodeB();
		}
	}
	...
	public function dropGuns($p, $block, $level){
		switch(mt_rand(1, 4)){
			...
			case 4:
				...
				$itemName = $item->setCustomName(TextFormat::AQUA . "CrossBow Bullet");
				$level->dropItem(new Vector3($block->x, $block->y, $block->z), $itemName);
			break;
	...
	public function onBuildBlocks(PlayerInteractEvent $e){
		...
		if($p instanceof Player){
			if($this->getPlayerMode($p) === 1){
				// WALL
				if($item->getId() == 5 && $item->getCustomName() == TextFormat:: AQUA . "Wall"){
					$e->setCancelled();
					$item = Item::get(5, 0, 1);
					$itemName = $item->setCustomName(TextFormat::AQUA . "Wall");
					$p->getInventory()->removeItem($itemName);
					$level = $this->plugin->getServer()->getLevelByName($this->data['arena']['arena_world']);
					$yaw = $p->yaw;
				...
				// STAIR
				if($item->getId() == 53 && $item->getCustomName() == TextFormat:: AQUA . "Stair"){
					$e->setCancelled();
					$item = Item::get(53, 0, 1);
					$itemName = $item->setCustomName(TextFormat::AQUA . "Stair");
					$p->getInventory()->removeItem($itemName);
					$level = $this->plugin->getServer()->getLevelByName($this->data['arena']['arena_world']);
					$yaw = $p->yaw;
    ...
    public function getStatus(){
        if($this->game === 0) return TextFormat::BLUE . "WAITING";
        if($this->game === 1) return TextFormat::RED . "IN-GAME";
    }
